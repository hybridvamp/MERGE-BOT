...
mergeApp = MergeBot(
    ...
    app_version="5.0+hybrid-mergebot",
)
    ...
    if m.from_user.id != int(Config.OWNER):
        if user.allowed is False:
            res = await m.reply_text(
                text=f"Hi **{m.from_user.first_name}**\n\n 🛡️ Unfortunately you can't use me\n\n**Login Password: https://t.me/hybridupdates/2**",
                quote=True,
            )
            return
    else:
    ...
    if user.merge_mode == 4:
        ...
        if rmess.video or rmess.document:
            ...
            if file_name is None:
                await m.reply("File name not found; goto @HybridUpdates")
                return
...
🔰 ᴏᴡɴᴇʀ ᴄᴀɴ ʙʀᴏᴀᴅᴄᴀsᴛ ᴍᴇssᴀɢᴇ ᴛᴏ ᴀʟʟ ᴜsᴇʀs
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                [InlineKeyboardButton("👨‍💻 Dev", url="https://t.me/HybridUpdates")],
